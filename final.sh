...
GREEN="\e[32m"
END="\e[0m"
net=$(curl -sI https://0xrobiul.me/ | head -n1 | awk '{printf $2}')
req="200"
if [[ ${net} == ${req} ]]
then
  ...
  then
        echo -e "${RED}[+]${GREEN} Installation Started${END}"
        pkg i unzip make golang python3 git &> /dev/null
